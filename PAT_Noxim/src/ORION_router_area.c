	...
	if (info->crossbar_model && info->crossbar_model < CROSSBAR_MAX_MODEL) {
		switch (info->crossbar_model) {
			...
			case TRISTATE_CROSSBAR:
				xb_in_len = info->n_switch_in * info->flit_width * CrsbarCellWidth;
				xb_out_len = info->n_switch_out * CrsbarCellHeight;
				router_area->crossbar = xb_in_len * xb_out_len * AreaTristate;
				break;
				*/
			default: printf ("error\n");  /* some error handler */
...
double SIM_router_area(SIM_router_area_t *router_area, int _switch)
{
	double AreaTemp = 0;
	switch (_switch) {
	case A_BUFFER:
		AreaTemp = router_area->buffer;
		break;
	case A_CROSSBAR:
		AreaTemp = router_area->crossbar;
		break;
	case A_VCALLOCATOR:
		AreaTemp = router_area->vc_allocator;
		break;
	case A_SWALLOCATOR:
		AreaTemp = router_area->sw_allocator;
		break;
	case A_TOTAL:
		AreaTemp = router_area->buffer + router_area->crossbar + router_area->vc_allocator + router_area->sw_allocator;
		break;
	}
	return AreaTemp;
}
