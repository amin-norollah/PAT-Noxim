...
int SIM_router_init(SIM_router_info_t *info, SIM_router_power_t *SIM_router_power, SIM_router_area_t *SIM_router_area, int num_vc, int buffer_size, double temperature)
{
	u_int line_width;
	...
	if(info->in_buf){
		...
		SIM_array_init(&info->in_buf_info, 1, PARM(in_buf_rport), 1, buffer_size, PARM(flit_width), outdrv, info->in_buffer_model);
	}
	...
	if(SIM_router_power){
		SIM_router_power_init(info, SIM_router_power, temperature);
	}
