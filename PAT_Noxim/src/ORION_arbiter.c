...
int SIM_arbiter_init(SIM_arbiter_t *arb, int arbiter_model, int ff_model, u_int req_width, double length, SIM_array_info_t *info, double Temperature)
{
	if ((arb->model = arbiter_model) && arbiter_model < ARBITER_MAX_MODEL) {
		...
		switch (arbiter_model) {
			case RR_ARBITER:
				...
				if (Temperature != 0) {
					/* temperature effect inversion : NOR */
					I_static += (6 * arb->req_width * ((WdecNORp*NOR2_TAB[0] / 90000 * FACTOR_LEAKAGE_CURRENT_P + WdecNORn*(NOR2_TAB[1] + NOR2_TAB[2] + NOR2_TAB[3]) / 90000 * FACTOR_LEAKAGE_CURRENT_N) / 4))* Temperature;

					/* temperature effect inversion : inverter */
					I_static += 2 * arb->req_width * ((Wdecinvn*NMOS_TAB[0] / 90000 * FACTOR_LEAKAGE_CURRENT_N + Wdecinvp*PMOS_TAB[0] / 90000 * FACTOR_LEAKAGE_CURRENT_P) / 2)* Temperature;
					/* temperature effect inversion : DFF */
					I_static += (arb->req_width * Wdff*DFF_TAB[0] / 90000 * FACTOR_LEAKAGE_CURRENT_N * 0.8)* Temperature;
				}
			...
			case MATRIX_ARBITER:
				...
				if (Temperature != 0) {
					/* temperature effect inversion : NOR */
					I_static += ((2 * arb->req_width - 1) * arb->req_width) * ((WdecNORp*NOR2_TAB[0] / 90000 * FACTOR_LEAKAGE_CURRENT_P + WdecNORn*(NOR2_TAB[1] + NOR2_TAB[2] + NOR2_TAB[3]) / 90000 * FACTOR_LEAKAGE_CURRENT_P) / 4)*Temperature;
					/* temperature effect inversion : inverter */
					I_static += arb->req_width * ((Wdecinvn*NMOS_TAB[0] / 90000 * FACTOR_LEAKAGE_CURRENT_N + Wdecinvp*PMOS_TAB[0] / 90000 * FACTOR_LEAKAGE_CURRENT_P) / 2)*Temperature;
					/* temperature effect inversion : DFF */
					I_static += (arb->req_width * (arb->req_width - 1) / 2) * (Wdff*DFF_TAB[0] / 90000 * FACTOR_LEAKAGE_CURRENT_N * 0.8)*Temperature;
				}
				arb->I_static = I_static;
			...
			case QUEUE_ARBITER:
				...
				return SIM_array_power_init(info, &arb->queue, Temperature);
				break;
...
double SIM_arbiter_stat_energy(SIM_arbiter_t *arb, SIM_array_info_t *info, double n_req, int max_avg)
{
	...
	switch (arb->model) {
		...
		case QUEUE_ARBITER:
			...
			Eavg = SIM_array_stat_energy(info, &arb->queue, n_req, n_grant, max_avg);
			break;
