...
int SIM_crossbar_init(SIM_crossbar_t *crsbar, int model, u_int n_in, u_int n_out, u_int in_seg, u_int out_seg, u_int data_width, u_int degree, int connect_type, int trans_type, double in_len, double out_len, double *req_len, double Temperature)
{
	...
	if ((crsbar->model = model) && model < CROSSBAR_MAX_MODEL) {
		...
		switch (model) {
			case MATRIX_CROSSBAR:
				...
				if (Temperature != 0) {
					/* temperature effect inversion : tri-state buffers */
					I_static += ((Woutdrvnandp * ((NAND2_TAB[0] + NAND2_TAB[1] + NAND2_TAB[2]) / 90000 * FACTOR_LEAKAGE_CURRENT_P) + Woutdrvnandn * NAND2_TAB[3] / 90000 * FACTOR_LEAKAGE_CURRENT_N) / 4 +
						(Woutdrvnorp * NOR2_TAB[0] / 90000 * FACTOR_LEAKAGE_CURRENT_P + Woutdrvnorn * (NOR2_TAB[1] + NOR2_TAB[2] + NOR2_TAB[3]) / 90000 * FACTOR_LEAKAGE_CURRENT_N) / 4 +
						Woutdrivern * NMOS_TAB[0] / 90000 * FACTOR_LEAKAGE_CURRENT_N + Woutdriverp * PMOS_TAB[0] / 90000 * FACTOR_LEAKAGE_CURRENT_P) * n_in * n_out * data_width * Temperature;
					/* temperature effect inversion : input driver */
					I_static += (Wdecinvn * NMOS_TAB[0] / 90000 * FACTOR_LEAKAGE_CURRENT_N + Wdecinvp * PMOS_TAB[0] / 90000 * FACTOR_LEAKAGE_CURRENT_P) * n_in * data_width * Temperature;
					/* temperature effect inversion : output driver */
					I_static += (Woutdrivern * NMOS_TAB[0] / 90000 * FACTOR_LEAKAGE_CURRENT_N + Woutdriverp * PMOS_TAB[0] / 90000 * FACTOR_LEAKAGE_CURRENT_P) * n_out * data_width * Temperature;
					/* temperature effect inversion : control signal inverter */
					I_static += (Wdecinvn * NMOS_TAB[0] / 90000 * FACTOR_LEAKAGE_CURRENT_N + Wdecinvp * PMOS_TAB[0] / 90000 * FACTOR_LEAKAGE_CURRENT_P) * n_in * n_out * Temperature;
				}
			...
			case MULTREE_CROSSBAR:
				...
				if (Temperature != 0) {
					...
					/* temperature effect inversion : output driver */
					I_static += (Woutdrivern * NMOS_TAB[0] / 90000 * FACTOR_LEAKAGE_CURRENT_N + Woutdriverp * PMOS_TAB[0] / 90000 * FACTOR_LEAKAGE_CURRENT_P) * n_out * data_width * Temperature;
					/* temperature effect inversion : mux */
					I_static += (WdecNORp * NOR2_TAB[0] / 90000 * FACTOR_LEAKAGE_CURRENT_P + WdecNORn * (NOR2_TAB[1] + NOR2_TAB[2] + NOR2_TAB[3]) / 90000 * FACTOR_LEAKAGE_CURRENT_N) / 4 * (2 * n_in - 1) * n_out * data_width * Temperature;
					/* temperature effect inversion : control signal inverter */
					I_static += (Wdecinvn * NMOS_TAB[0] / 90000 * FACTOR_LEAKAGE_CURRENT_N + Wdecinvp * PMOS_TAB[0] / 90000 * FACTOR_LEAKAGE_CURRENT_P) * n_in * n_out * Temperature;
				}
...
double SIM_crossbar_stat_energy(SIM_crossbar_t *crsbar, int max_avg, double n_data)
{
