...
int SIM_array_power_init( SIM_array_info_t *info, SIM_array_t *arr, double temperature)
{
	...
	if(info->arr_buf_type == SRAM){
		...
		if (dec_width = SIM_logtwo(info->n_set)) {
			...
			arr->I_static += arr->data_bitline.I_static / 90000 * FACTOR_LEAKAGE_CURRENT_P * 0.8 * cols * info->write_ports * temperature;
			...
			/* temperature effect inversion */
			arr->I_static += arr->data_bitline_pre.I_static / 90000 * FACTOR_LEAKAGE_CURRENT_P * 0.8 * cols * info->read_ports* temperature;
		...
		/* temperature effect inversion */
		arr->I_static += arr->data_wordline.I_static / 90000 * FACTOR_LEAKAGE_CURRENT_P * 0.8 * rows * ports* temperature;
		...
		/* temperature effect inversion */
		arr->I_static += arr->data_mem.I_static / 90000 * FACTOR_LEAKAGE_CURRENT_P * 0.8 * rows * ports * temperature;
		...
		/* temperature effect inversion */
		arr->I_static = arr->ff.I_static / 90000 * FACTOR_LEAKAGE_CURRENT_P * 0.8 * info->data_width * info->n_set * temperature;
	}
	return 0;
}
