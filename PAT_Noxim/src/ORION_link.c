...
void getOptBufferingRtoR(int *k, double *h, double Length) {
	if (PARM(shielding)) {
		double r = computeResistance(Length)*ResistanceFactor;
		double c_g = 2 * computeGroundCapacitance(Length)*CapacitanceFactor;
		double c_c = 2 * computeCouplingCapacitance(Length)*CapacitanceFactor;
		...
		*k = (int)sqrt(((0.4*r*c_g) + (1.51*r*c_c)) /
			(0.7*BufferDriveResistance*BufferInputCapacitance));
		*h = sqrt(((0.7*BufferDriveResistance*c_g) + (1.4*2.2*BufferDriveResistance*c_c)) /
			(27.7*r*BufferInputCapacitance));
...
double LinkDynamicEnergyPerBitPerMeter(double Length, double vdd) 
{
	...
	getOptBufferingRtoR(ptr_k, ptr_h, Length);
    double bufferC = ((double)k) * (BufferInputCapacitance) * h;

    return (((totalWireC + bufferC) * vdd * vdd)/ Length)*2.23e-2;
}
...
double LinkLeakagePowerPerMeter(double Length, double vdd) 
{
    ...
    return ((vdd*(pmosLeakage + nmosLeakage)/2)/ Length)*0.85e-6;
}
