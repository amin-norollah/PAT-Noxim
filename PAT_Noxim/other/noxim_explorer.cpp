...
#define AVG_DELAY_LABEL       "  Global average delay 		: "
#define AVG_THROUGHPUT_LABEL  "  Global average throughput	: "
#define THROUGHPUT_LABEL      "  Throughput 			: "
#define MAX_DELAY_LABEL       "  Max delay 			: "

#define TOTAL_ENERGY_LABEL    "  + Total energy 		: "
#define CORES_ENERGY_LABEL    "    Cores energy 		: "
#define ROUTERS_ENERGY_LABEL    "    Routers energy 		: "
#define TOTAL_POWER_LABEL    "  + Avg power 			: "
#define CORES_POWER_LABEL    "    Avg cores power		: "
#define ROUTERS_POWER_LABEL    "    Avg routers power 		: "
#define R_TOTAL_POWER_LABEL    "  + Avg power per router	: "
#define R_STATIC_POWER_LABEL    "    Static power 		: "
#define R_DYNAMIC_POWER_LABEL    "    Avg dynamic power		: "
...
#define TOTAL_LABEL_NUM			15
#define MATLAB_VAR_NAME			"data"
#define MATRIX_COLUMN_WIDTH		20
...
struct TSimulationResults
{
  ...
  double       cores_energy;
  double       routers_energy;
  double       total_power;
  double       cores_power;
  double       routers_power;
  double       r_total_power;
  double       r_static_power;
  double       r_dynamic_power;
};

//---------------------------------------------------------------------------

string conv_string(int value) {
	stringstream sstr;
	sstr << value;
	return sstr.str();
}
		 ...
		 string& error_msg)
{
  ...
  while (!fin.eof())
      ...
      if (pos != string::npos) 
	{
	  ...
	  /*       added by A.Norollah        */
	  pos = line.find(CORES_ENERGY_LABEL);
	  if (pos != string::npos)
	  {
		  nread++;
		  istringstream iss(line.substr(pos + string(CORES_ENERGY_LABEL).size()));
		  iss >> sres.cores_energy;
		  continue;
	  }

	  pos = line.find(ROUTERS_ENERGY_LABEL);
	  if (pos != string::npos)
	  {
		  nread++;
		  istringstream iss(line.substr(pos + string(ROUTERS_ENERGY_LABEL).size()));
		  iss >> sres.routers_energy;
		  continue;
	  }

	  pos = line.find(TOTAL_POWER_LABEL);
	  if (pos != string::npos)
	  {
		  nread++;
		  istringstream iss(line.substr(pos + string(TOTAL_POWER_LABEL).size()));
		  iss >> sres.total_power;
		  continue;
	  }

	  pos = line.find(CORES_POWER_LABEL);
	  if (pos != string::npos)
	  {
		  nread++;
		  istringstream iss(line.substr(pos + string(CORES_POWER_LABEL).size()));
		  iss >> sres.cores_power;
		  continue;
	  }

	  pos = line.find(ROUTERS_POWER_LABEL);
	  if (pos != string::npos)
	  {
		  nread++;
		  istringstream iss(line.substr(pos + string(ROUTERS_POWER_LABEL).size()));
		  iss >> sres.routers_power;
		  continue;
	  }

	  pos = line.find(R_TOTAL_POWER_LABEL);
	  if (pos != string::npos)
	  {
		  nread++;
		  istringstream iss(line.substr(pos + string(R_TOTAL_POWER_LABEL).size()));
		  iss >> sres.r_total_power;
		  continue;
	  }

	  pos = line.find(R_STATIC_POWER_LABEL);
	  if (pos != string::npos)
	  {
		  nread++;
		  istringstream iss(line.substr(pos + string(R_STATIC_POWER_LABEL).size()));
		  iss >> sres.r_static_power;
		  continue;
	  }

	  pos = line.find(R_DYNAMIC_POWER_LABEL);
	  if (pos != string::npos)
	  {
		  nread++;
		  istringstream iss(line.substr(pos + string(R_DYNAMIC_POWER_LABEL).size()));
		  iss >> sres.r_dynamic_power;
		  continue;
	  }
	  /*       added by A.Norollah        */
	...
	fout << setw(MATRIX_COLUMN_WIDTH) << ExtractFirstField(aggr_conf[i].second); // this fix the problem with pir
	 ...
	 /*  << setw(MATRIX_COLUMN_WIDTH) << sres.ni_delay 
	   ...
	   << setw(MATRIX_COLUMN_WIDTH) << sres.total_energy
		  << setw(MATRIX_COLUMN_WIDTH) << sres.cores_energy			//added A.Norollah
		  << setw(MATRIX_COLUMN_WIDTH) << sres.routers_energy		//added A.Norollah
		  << setw(MATRIX_COLUMN_WIDTH) << sres.total_power			//added A.Norollah
		  << setw(MATRIX_COLUMN_WIDTH) << sres.cores_power			//added A.Norollah
		  << setw(MATRIX_COLUMN_WIDTH) << sres.routers_power		//added A.Norollah
		  << setw(MATRIX_COLUMN_WIDTH) << sres.r_total_power		//added A.Norollah
		  << setw(MATRIX_COLUMN_WIDTH) << sres.r_static_power		//added A.Norollah
		  << setw(MATRIX_COLUMN_WIDTH) << sres.r_dynamic_power		//added A.Norollah
			      ...
			      ofstream& fout, string& error_msg)
{
  ...
  fout << setw(MATRIX_COLUMN_WIDTH) << "avg_delay"
    /*   << setw(MATRIX_COLUMN_WIDTH) << "NI delay"
	   ...
	   << setw(MATRIX_COLUMN_WIDTH) << "cores_energy"			//added A.Norollah
	   << setw(MATRIX_COLUMN_WIDTH) << "routers_energy"			//added A.Norollah
	   << setw(MATRIX_COLUMN_WIDTH) << "avg_power"				//added A.Norollah
	   << setw(MATRIX_COLUMN_WIDTH) << "avg_cores_power"		//added A.Norollah
	   << setw(MATRIX_COLUMN_WIDTH) << "avg_routers_power"		//added A.Norollah
	   << setw(MATRIX_COLUMN_WIDTH) << "power_per_router"		//added A.Norollah
	   << setw(MATRIX_COLUMN_WIDTH) << "static_per_router"		//added A.Norollah
	   << setw(MATRIX_COLUMN_WIDTH) << "dynamic_per_router"		//added A.Norollah
		   ...
		   ofstream& fout, string& error_msg)
{
  fout << var_name << " = [];" << endl
       ...
       << "   tmp = " << MATLAB_VAR_NAME << "(ifirst:ilast, cols-"<< TOTAL_LABEL_NUM-1<<"+" << column << ");" << endl
       << "   avg = mean(tmp);" << endl
       << "   [h sig ci] = ttest(tmp, 0.1);" << endl
       << "   ci = (ci(2)-ci(1))/2;" << endl
       << "   " << var_name << " = [" << var_name << "; " << MATLAB_VAR_NAME << "(ifirst, 1:cols-"<< TOTAL_LABEL_NUM - 1 <<"), avg ci];" << endl
       << "end" << endl
       << endl;
  ...
  for (uint_new i=0; i<conf_space.size(); i++)
    {
      ...
      string   fname  = mfname + conv_string(timeinfo->tm_mday) + "_" + conv_string(timeinfo->tm_hour) + "_" + conv_string(timeinfo->tm_min) + "_" + conv_string(timeinfo->tm_sec) + "_" + ".m";
      ofstream fout;
